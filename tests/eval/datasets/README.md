...
This directory contains StudyOps Agent evaluation datasets for testing capstone
behavior.

Current datasets:

- `basic-dataset.json`: default StudyOps cases for `agents-cli eval generate`.
- `studyops-capstone-dataset.json`: broader prompts for capstone and regression checks.
...
### Custom Dataset
```bash
# Generate traces for the capstone dataset
agents-cli eval generate --dataset tests/eval/datasets/studyops-capstone-dataset.json --output custom_traces/
agents-cli eval grade --config tests/eval/eval_config.yaml --traces custom_traces/
```
